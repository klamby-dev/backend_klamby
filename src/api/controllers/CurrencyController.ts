...
export class CurrencyController {
    ...
    public async addCurrency(@Body({ validate: true }) currencyParameter: CreateCurrency, @Res() response: any): Promise<any> {
        ...
        newCurrency.value = currencyParameter.value;
    ...
    @Get('/currencylist')
    @Authorized()
    public async currencyList(@QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('keyword') keyword: string, @QueryParam('status') status: number, @QueryParam('count') count: number | boolean, @Res() response: any): Promise<any> {
        const select = ['currencyId', 'title', 'code', 'symbolLeft', 'symbolRight', 'value', 'modifiedDate', 'createdDate', 'isActive'];
        const search: any = [
            {
                name: 'title',
                op: 'like',
