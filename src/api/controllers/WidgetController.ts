    ...
    public async createWidget(@Body({ validate: true }) widgetParam: CreateWidget, @Res() response: any): Promise<any> {
        if ((widgetParam.refId).length > 0) {
            ...
            const errResponse: any = {
                status: 1,
                message: 'Widget refId is Manditory .',
        ...
        if (widget.widgetLinkType === 2) {
            for (const val of value) {
                const product = await this.productService.findOne({
                    select: ['productId', 'sku', 'name', 'quantity', 'price', 'isActive'],
                    where: {
                        productId: val.refId,
                    },
                });
            ...
            for (const val of value) {
                ...
                const sort = [];
                const vendorCategoryList = await this.categoryPathService.listByQueryBuilder(1, 0, select, whereCondition, searchConditions, relations, groupBy, sort, false, true);
