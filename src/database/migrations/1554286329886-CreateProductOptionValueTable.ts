...
export class CreateProductOptionValueTable1554286329886 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<any> {
        ...
        const ifTable = await queryRunner.getTable('product_option_value');
        const ifDataExsist = ifTable.foreignKeys.find(fk => fk.columnNames.indexOf('product_id') !== -1);
        if (!ifDataExsist) {
            await queryRunner.createForeignKey(ifTable, this.tableForeignKey);
        }
    }
    ...
    public async down(queryRunner: QueryRunner): Promise<any> {
        ...
        const ifDataExsist = ifTable.foreignKeys.find(fk => fk.columnNames.indexOf('product_id') !== -1);
        if (ifDataExsist) {
            await queryRunner.dropForeignKey(ifTable, this.tableForeignKey);
        }
    }
}
